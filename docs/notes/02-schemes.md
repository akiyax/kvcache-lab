...
这两个词必须分清，否则会得出错误的卸载结论：

| | 旧的 KV 能扔掉吗 | 总容量需求 |
|---|---|---|
| 滑窗 SWA | **能真扔**，超出窗口即删 | 封顶为常数 |
| 稀疏 | **不能扔**，任何块都可能被选中 | 随上下文增长 |

稀疏的「不能扔」指的是**必须保留**，但**没有规定保留在哪一层**——CPU 内存、SSD、
对象存储都可以。这与「必须驻留显存」是两回事。

而稀疏减少的「读带宽」，指的是**显存 → 计算单元**这一层：decode 每步本需把整个
KV 读一遍，稀疏让它只读被选中的那部分。这个收益在 KV 全部驻留显存时就已成立，
与外部存储无关。
...
| 稀疏 | 每次只读一部分 | 总量不减但天然分冷热；难点转为**访问随机化** |
| 线性混合 | 部分层用固定状态 | 大幅减少需卸载的层数 |

稀疏那一行值得展开——它对卸载是把**双刃剑**：

- **有利**：每步只读一小部分，其余是「冷」的，天然适合放到下层存储
- **不利**：选哪些块是每步动态算的，**预取无从下手**。远程取一次的延迟以毫秒计，
  而一个 decode 步的总预算也才十毫秒量级

**顺序访问变成了随机访问**——这在对象存储上是两种完全不同的性能特征。因此稀疏模型
的卸载难点不在「带宽够不够」，而在「**能否预测下一步要读哪些块**」，解法会偏向
预测预取、热块常驻、块粒度调优，而非单纯堆带宽。
