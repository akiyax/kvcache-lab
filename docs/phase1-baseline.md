...
| 指标 | 不命中 | 命中 | 变化 |
|------|-------|------|------|
| TTFT p50 (ms) | 4632.3 | **2185.9** | **−52.8%** |
| TTFT p90 (ms) | 5666.1 | 2736.3 | −51.7% |
| ITL p50 (ms) | 22.89 | **9.52** | −58.4% |
| 吞吐 (output tok/s) | 146.4 | **288.4** | **+97.0%** |
| 吞吐 (req/s) | 1.14 | 2.27 | +99.1% |
| 总耗时 (s) | 27.99 | 14.08 | −49.7% |

两次均为 32 请求 / 并发 8，全部成功；输入 93,838 vs 92,984 token（差异来自唯一标记）。

说明：

- **TTFT 减半**符合预期：命中后跳过约 99.4% 前缀的 prefill 计算。
- **ITL 同样大幅下降**（22.89 → 9.52 ms）值得注意。vLLM 默认开启 chunked prefill，
  prefill 分块与 decode 交错争抢 GPU；命中后 prefill 工作量骤减，decode 得以更连续
  地执行。这印证了 [04-serving.md](notes/04-serving.md) 的判断——卸载/缓存对吞吐的
  改善是**间接**的，直接受益的是 TTFT。
- 共享模式下最初几个请求仍需 prefill（并发 8 同时启动，彼此看不到对方的缓存写入），
  故 +97% 是含冷启动的**混合结果**，非稳态命中的理论上限。
- TTFT 绝对值偏高是**并发 8 一次性打满**导致的排队，非单请求延迟。
- 压测脚本零依赖，本次从 **Windows 侧**直接压 WSL 内的服务。

> 早期曾用「两句话重复十余次」的合成填充文本测得 249.9 tok/s（输入仅 676 token）。
> 该数据已废弃：负载不符合 `design.md` 规定的长文档问答场景，且输入过短导致 KV
> 规模可忽略，测不出缓存与带宽的影响。
...
- [ ] 测量稳态命中率下的收益上限（预热缓存后再压测，排除冷启动影响）
- [ ] 从 vLLM metrics 端点读取真实的前缀缓存命中率，与 TTFT 变化交叉验证
